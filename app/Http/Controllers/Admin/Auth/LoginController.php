...
class LoginController extends Controller
{
  ...
  public function reSendOtp(Request $request,$user_id,$otp_type)
  {
    ...
    if ($otp_type==1) {
      $data = array( 'email' => $user_rs[0]->email_id, 'otp' => $rs_otp, 'from' => 'info@IAMRF.in', 'from_name' => 'IAMRF' );
      ...
      $message = $rs_otp.' is the OPT Verification code for IAMRF. SIR SALASAR BALAJI ENTERPRISES PRIVATE LIMITED';
  ...
  public function registerStore(Request $request)
  {
    try {
      ...
      $message = $mobile_otp.' is the OPT Verification code for IAMRF. SIR SALASAR BALAJI ENTERPRISES PRIVATE LIMITED';
      $tempid ='1707163860074623221'; 
  ...
  public function forgotPasswordSendLink(Request $request)
  {
    ...
    if (!empty($user)) {
      if ($user[0]->status==0) {
        ...
        $data = array( 'link' => route('admin.forgot.password.reset',Crypt::encrypt($user[0]->email_id)),'user_name' => $user[0]->name,'email' => $user[0]->email_id, 'from' => 'info@IAMRF.in', 'from_name' => 'IAMRF' );
        Mail::send('emails.forgotPassword', $data, function( $message ) use ($data)
        {
          $message->to($data['email'] )->from( $data['from'], 'IAMRF' )->subject( 'Forgot Password' );
        });
      }
